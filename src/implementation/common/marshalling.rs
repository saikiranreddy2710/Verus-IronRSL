...
impl<K , V> Marshalable for HashMap<K,V>
where
  ...
  open spec fn view_equal(&self, other: &Self) -> bool {
    ...
    let o1 = other.keys().cloned().collect::<Vec<K>>()@;
    let s2 = self.values().cloned().collect::<Vec<V>>()@;
    let o2 = other.values().cloned().collect::<Vec<V>>()@;

    s1.len() == o1.len()
    && s2.len() == o2.len()
    && (forall |i: int| 0 <= i < s1.len() ==> #[trigger] s1[i].view_equal(&o1[i]))
    && (forall |i: int| 0 <= i < s2.len() ==> #[trigger] s2[i].view_equal(&o2[i]))
  }
  ...
  open spec fn ghost_serialize(&self) -> Seq<u8> {
    (self@.len() as usize).ghost_serialize()
      + self.keys().cloned().collect::<Vec<K>>()@.fold_left(Seq::<u8>::empty(), |acc: Seq<u8>, x: K| acc + x.ghost_serialize())
      + self.values().cloned().collect::<Vec<V>>()@.fold_left(Seq::<u8>::empty(), |acc: Seq<u8>, x: V| acc + x.ghost_serialize())
  }
  ...
  exec fn serialize(&self, data: &mut Vec<u8>)
  {
    ...
    proof {
      assert(
        data@.subrange(old(data)@.len() as int, data@.len() as int) == self.len().ghost_serialize() +
            self.keys().cloned().collect::<Vec<K>>()@.subrange(0, i as int).fold_left(Seq::<u8>::empty(), |acc: Seq<u8>, x: K| acc + x.ghost_serialize()) + self.values().cloned().collect::<Vec<V>>()@.subrange(0, i as int).fold_left(Seq::<u8>::empty(), |acc: Seq<u8>, x: V| acc + x.ghost_serialize())
      );
    }

    let keys = self.keys().cloned().collect::<Vec<K>>();
    let values = self.values().cloned().collect::<Vec<V>>();
    ...
    while i < keys.len()
      ...
      //   keys.len().ghost_serialize() +
      //   keys@.subrange(0, i as int).fold_left(Seq::<u8>::empty(), |acc: Seq<u8>, x: K| acc + x.ghost_serialize()) +
      //   values@.subrange(0, i as int).fold_left(Seq::<u8>::empty(), |acc: Seq<u8>, x: V| acc + x.ghost_serialize()),
      forall |x: K| keys@.contains(x) ==> #[trigger] x.is_marshalable(),
      forall |x: V| values@.contains(x) ==> #[trigger] x.is_marshalable(),
      data@.len() >= old(data)@.len(),
    {
    ...
    proof {
      assert(self.keys().cloned().collect::<Vec<K>>()@ == self.keys().cloned().collect::<Vec<K>>()@.subrange(0, self@.len() as int));
      assert(self.values().cloned().collect::<Vec<V>>()@ == self.values().cloned().collect::<Vec<V>>()@.subrange(0, self@.len() as int));
    }
    ...
    // req, ens from trait
  {
    ...
    let mut res: HashMap<K,V> = HashMap::with_capacity(len);
    let mut i: usize = 0;
    let mut end = mid;
        ...
        // accf@ == |acc: Seq<u8>, x: K| acc + x.ghost_serialize(),
    {
      ...
      let (v, end2) = match V::deserialize(data, end1) { None => {
        return None;
      }, Some(x) => x, };
    ...
    ::builtin_macros::verus! {
      impl $(< $($poly: Marshalable),* >)? Marshalable for $newstruct $(< $($poly),* >)? {
        ...
        exec fn _is_marshalable(&self) -> bool
          // req, ens from trait
        {
          true $(&& self.$field._is_marshalable())*
